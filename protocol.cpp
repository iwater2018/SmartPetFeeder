...
extern Servo Servo1;
extern Servo Servo2;
extern uint8_t status_7;
extern void servo_action(Servo * sv) ;
extern void servo_action(Servo * sv1, Servo * sv2) ;
extern void toggle_value(uint8_t * st);
extern void buzzer_echo();
...
static unsigned char dp_download_quick_feed_handle(const unsigned char value[], unsigned short length)
{
    ...
    Serial.println(quick_feed);
    if(quick_feed == 0) {
        //开关关
        Serial.println("Tuya: Quick Feed Off!");
    }else {
        //开关开
        servo_action(&Servo1);
        buzzer_echo();
        Serial.println("Tuya: Quick Feed On!");
...
static unsigned char dp_download_manual_feed_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    Serial.println("Tuya: manual feed!");
    Serial.println(manual_feed);

    // if(manual_feed == 2) {
        servo_action(&Servo1, &Servo2);
        buzzer_echo();
...
static unsigned char dp_download_light_handle(const unsigned char value[], unsigned short length)
{
    ...
    if(light == 0) {
        //开关关
        Serial.println("Tuya: Light Off!");
        toggle_value(&status_7);
        buzzer_echo();
        Servo1.write(0);
        Servo2.write(0);
    }else {
        //开关开
        Serial.println("Tuya: Light On!");
        toggle_value(&status_7);
        buzzer_echo();
        Servo1.write(0);
        Servo2.write(0);
